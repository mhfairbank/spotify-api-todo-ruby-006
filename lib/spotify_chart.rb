...
class SpotifyChart
  ...
  def most_streamed(region)
    ...
    fetch_track_album_artist(JSON.load(open(get_url("most_streamed",region))))
    #fetch_track_album_artist(../spec/support/us_most_streamed.json)
  end
  ...
  def most_shared(region)
    ...
    # finally, call on fetch_track_album_artist using the 
    # hash that get_json returns
    fetch_track_album_artist(JSON.load(open(get_url("most_shared",region))))
  end
